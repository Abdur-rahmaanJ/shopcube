...
from modules.box__ecommerce.inventory.models import InventoryCount, InventoryCountItem, Location, StockTransfer, StockTransferItem
...
class TestInventoryAccess:
    def test_requires_login(self, test_client):
        response = test_client.get("/inventory/dashboard", follow_redirects=True)
        assert response.status_code == 200

    def test_dashboard_admin(self, test_client, login_admin_user):
        response = test_client.get(url_for("inventory.dashboard"))
        assert response.status_code == 200
...
class TestInventoryCounts:
    def test_create_count_sheet(self, test_client, login_admin_user, db_session):
        ...
        db_session.add(prod)
        db_session.commit()
        response = test_client.post(url_for("inventory.new"), data={"notes": "Test count"}, follow_redirects=True)
        assert response.status_code == 200
    ...
    def test_complete_count_adjusts_stock(self, test_client, login_admin_user, db_session):
        ...
        ic.update()
        response = test_client.post(url_for("inventory.count", count_id=ic.id), follow_redirects=True)
        assert response.status_code == 200
...
class TestLocations:
    def test_add_location(self, test_client, login_admin_user):
        response = test_client.post(
            url_for("inventory.location_add"),
    ...
    def test_delete_location(self, test_client, login_admin_user):
        ...
        response = test_client.post(url_for("inventory.location_delete", loc_id=loc.id), follow_redirects=True)
        assert response.status_code == 200
...
class TestStockTransfers:
    def test_create_transfer(self, test_client, login_admin_user, db_session):
        ...
        response = test_client.post(
            url_for("inventory.transfer_create"),
    ...
    def test_transfer_complete_deducts_stock(self, test_client, login_admin_user, db_session):
        ...
        response = test_client.post(url_for("inventory.transfer_complete", t_id=t.id), follow_redirects=True)
        assert response.status_code == 200
        prod = Product.query.get(prod.id)
