...
class TestPosTransaction:
    def test_transaction_empty_items_allowed(self, test_client, db_session):
        ...
        response = test_client.post(url_for("shopyo_ecommerce.pos.transaction"), json={"items": {}, "amount_paid": 10})
    ...
    def test_transaction_invalid_product(self, test_client, db_session):
        login_admin(test_client)
        response = test_client.post(url_for("shopyo_ecommerce.pos.transaction"), json={"items": {"NONEXISTENT": {"count": 1}}, "amount_paid": 10})
    ...
    def test_transaction_success(self, test_client, db_session):
        ...
        db_session.add(prod)
        db_session.commit()
        response = test_client.post(url_for("shopyo_ecommerce.pos.transaction"), json={"items": {"T001": {"count": 2}}, "amount_paid": 25})
        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
    ...
    def test_transaction_insufficient_payment(self, test_client, db_session):
        ...
        db_session.add(prod)
        db_session.commit()
        response = test_client.post(url_for("shopyo_ecommerce.pos.transaction"), json={"items": {"T002": {"count": 3}}, "amount_paid": 10})
        assert response.status_code == 400
    ...
    def test_transaction_with_discount(self, test_client, db_session):
        ...
        db_session.add(prod)
        db_session.commit()
        response = test_client.post(url_for("shopyo_ecommerce.pos.transaction"), json={"items": {"T003": {"count": 2}}, "amount_paid": 200, "discount_type": "percentage", "discount_value": 10})
        assert response.status_code == 200
    ...
    def test_transaction_with_notes(self, test_client, db_session):
        ...
        db_session.add(prod)
        db_session.commit()
        response = test_client.post(url_for("shopyo_ecommerce.pos.transaction"), json={"items": {"T004": {"count": 1}}, "amount_paid": 10, "notes": "Test note"})
        assert response.status_code == 200
        tx = Transaction.query.first()
...
class TestPosShifts:
    def test_shift_open_close(self, test_client, db_session):
        login_admin(test_client)
        response = test_client.post(url_for("shopyo_ecommerce.pos.shift_open"), data={"starting_cash": 100}, follow_redirects=True)
        assert response.status_code == 200
        s = Shift.query.filter_by(status="open").first()
        assert s is not None
        assert float(s.starting_cash) == 100
        test_client.post(url_for("shopyo_ecommerce.pos.shift_close", shift_id=s.id), data={"actual_cash": 500}, follow_redirects=True)
        s = Shift.query.get(s.id)
        assert s.status == "closed"

    def test_shift_double_open_denied(self, test_client, db_session):
        login_admin(test_client)
        test_client.post(url_for("shopyo_ecommerce.pos.shift_open"), data={"starting_cash": 0}, follow_redirects=True)
        response = test_client.post(url_for("shopyo_ecommerce.pos.shift_open"), data={"starting_cash": 0}, follow_redirects=True)
        assert response.status_code == 200
...
class TestPosQuickKeys:
    def test_quick_key_add_delete(self, test_client, db_session):
        ...
        db_session.add(prod)
        db_session.commit()
        response = test_client.post(url_for("shopyo_ecommerce.pos.quick_key_add"), data={"product_id": prod.id, "position": 1, "label": "Q1"}, follow_redirects=True)
        assert response.status_code == 200
        qk = QuickKey.query.first()
        assert qk is not None
        assert qk.position == 1
        test_client.post(url_for("shopyo_ecommerce.pos.quick_key_delete", key_id=qk.id), follow_redirects=True)
...
class TestPosReturns:
    def test_return_process(self, test_client, db_session):
        ...
        db_session.commit()
        response = test_client.post(url_for("shopyo_ecommerce.pos.process_return", tx_id=tx.id), follow_redirects=True)
        assert response.status_code == 200
