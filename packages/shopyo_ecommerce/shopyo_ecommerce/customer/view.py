...
from shopyo_ecommerce.shop.forms import RegisterCustomerForm
...
def register():
    if request.method == "POST":
        ...
        return mhelp.redirect_url("shopyo_ecommerce.shop.homepage")
...
def dashboard():
    ...
    context.update({"_hide_nav": True, "_logout_url": url_for("shopyo_ecommerce.customer.logout")})
...
def order_view(order_id):
    ...
    if order.logged_in_customer_email:
        if order.logged_in_customer_email != current_user.email:
            ...
            return redirect(url_for("shopyo_ecommerce.customer.orders"))
    else:
...
def admin_group_add():
    ...
    return redirect(url_for("shopyo_ecommerce.customer.admin_dashboard"))
