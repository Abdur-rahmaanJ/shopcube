...
class TestVendorCrud:
    def test_vendor_add(self, test_client, db_session):
        ...
        response = test_client.post(url_for("vendor.add"), data={"name": "Test Vendor", "email": "v@t.com"}, follow_redirects=True)
    ...
    def test_vendor_add_empty_name(self, test_client, db_session):
        login_admin(test_client)
        test_client.post(url_for("vendor.add"), data={"name": ""}, follow_redirects=True)
        assert Vendor.query.count() == 0

    def test_vendor_edit(self, test_client, db_session):
        login_admin(test_client)
        v = Vendor(name="Old Name").insert()
        test_client.post(url_for("vendor.edit", vendor_id=v.id), data={"name": "Updated Name"}, follow_redirects=True)
    ...
    def test_vendor_delete(self, test_client, db_session):
        ...
        test_client.post(url_for("vendor.delete", vendor_id=v.id), follow_redirects=True)
    ...
    def test_vendor_list(self, test_client, db_session):
        ...
        response = test_client.get(url_for("vendor.dashboard"))
