...
from shopyo_ecommerce import productphotos
...
from shopyo_ecommerce.product.models import StockAdjustment
from shopyo_ecommerce.product.models import BundleComponent
...
def add(subcategory_id):
    ...
    if request.method == "POST":
        ...
        if has_product is False:
            ...
            subcategory.update()
            return redirect(
                url_for("shopyo_ecommerce.product.add_dashboard", subcategory_id=subcategory_id)
            )
...
def delete(barcode):
    ...
    product.delete()
    db.session.commit()
    return redirect(url_for("shopyo_ecommerce.product.list", subcategory_id=subcategory.id))
...
def image_delete(filename, barcode):
    ...
    return redirect(url_for("shopyo_ecommerce.product.edit_dashboard", barcode=barcode))
