...
from shopyo_ecommerce import categoryphotos
from shopyo_ecommerce import productexcel
from shopyo_ecommerce import subcategoryphotos
from shopyo_ecommerce._utils import is_safe_path_component
from shopyo_ecommerce._utils import unique_sec_filename

from shopyo_settings.helpers import get_setting
from shopyo_ecommerce.category.forms import UploadProductForm
from shopyo_ecommerce.category.models import Category
from shopyo_ecommerce.category.models import SubCategory
from shopyo_ecommerce.product.models import Color
from shopyo_ecommerce.product.models import Product
from shopyo_ecommerce.product.models import Size
from shopyo_ecommerce.resource.models import Resource
...
def add():
    ...
    if request.method == "POST":
        ...
        if is_empty_str(name):
            ...
            return redirect(url_for("shopyo_ecommerce.category.add"))
...
def delete(name):
    ...
    if is_empty_str(name):
        ...
        return redirect(url_for("shopyo_ecommerce.category.dashboard"))
...
def add_sub(category_name):
    if request.method == "POST":
        ...
        category.update()
    return redirect(url_for("shopyo_ecommerce.category.manage_sub", category_name=category_name))
...
def edit_sub_name(subcategory_id):
    if request.method == "POST":
        ...
        return redirect(
            url_for("shopyo_ecommerce.category.manage_sub", category_name=subcategory.category.name)
        )
...
def subcategory_image_delete(subcategory_id, filename):
    ...
    return redirect(
        url_for("shopyo_ecommerce.category.edit_sub_img_dashboard", subcategory_id=subcategory_id)
    )
...
def upload_check():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            flash_errors(form)
    return redirect(url_for("shopyo_ecommerce.category.upload"))
