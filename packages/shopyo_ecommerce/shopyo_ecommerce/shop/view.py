...
from shopyo_ecommerce.shop.cart import Cart
...
from shopyo_ecommerce.category.models import SubCategory
from shopyo_ecommerce.product.models import Product
from shopyo_ecommerce.shop.forms import CheckoutForm
from shopyo_ecommerce.shop.helpers import get_cart_data
from shopyo_ecommerce.shop.helpers import get_min_max_subcateg
from shopyo_ecommerce.shop.models import BillingDetail
from shopyo_ecommerce.shop.models import Order
from shopyo_ecommerce.shop.models import OrderItem
from shopyo_ecommerce.shopman.models import Coupon
from shopyo_ecommerce.shopman.models import DeliveryOption
from shopyo_ecommerce.shopman.models import PaymentOption
...
def cart_add(product_barcode):
    if request.method == "POST":
        ...
        if Cart.add(barcode, item_info):
            return mhelp.redirect_url("shopyo_ecommerce.shop.product", product_barcode=barcode)
        else:
            flash(
                notify_warning(
            ...
            return redirect(url_for("shopyo_ecommerce.shop.product", product_barcode=barcode))
...
def cart_remove(product_barcode, size, color):
    if "cart" in session:
        ...
        return mhelp.redirect_url("shopyo_ecommerce.shop.cart")
...
def checkout():
    ...
    countries_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "shopman", "data", "country.json")
...
def checkout_process():
    if request.method == "POST":
        ...
        if len(cart_info["cart_data"]) == 0:
            ...
            return mhelp.redirect_url("shopyo_ecommerce.shop.checkout")
    ...
    if request.method == "POST":
        if login_form.validate_on_submit():
            ...
            if user is None or not user.check_hash(password):
                ...
                return redirect(url_for("shopyo_ecommerce.shop.checkout"))
            login_user(user)
            return redirect(url_for("shopyo_ecommerce.shop.checkout"))
