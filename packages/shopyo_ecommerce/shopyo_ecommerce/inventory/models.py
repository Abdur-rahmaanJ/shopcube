from shopyo.api.models import PkModel
from init import db


class Location(PkModel):
    __tablename__ = "shopyo_ecommerce_locations"
    name = db.Column(db.String(200), nullable=False)
    address = db.Column(db.Text)
    is_active = db.Column(db.Boolean, default=True)

    def __repr__(self):
        return f"Location: {self.name}"

    def insert(self):
        db.session.add(self)
        db.session.commit()
        return self


class StockPerLocation(PkModel):
    __tablename__ = "shopyo_ecommerce_stock_per_location"
    product_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_product.id"), nullable=False)
    location_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_locations.id"), nullable=False)
    quantity = db.Column(db.Integer, default=0)

    product = db.relationship("Product", lazy=True)
    location = db.relationship("Location", lazy=True)


class StockTransfer(PkModel):
    __tablename__ = "shopyo_ecommerce_stock_transfers"
    from_location_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_locations.id"), nullable=False)
    to_location_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_locations.id"), nullable=False)
    created_at = db.Column(db.DateTime, default=db.func.now())
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)
    status = db.Column(db.String(20), default="draft")
    notes = db.Column(db.Text)

    from_location = db.relationship("Location", foreign_keys=[from_location_id], lazy=True)
    to_location = db.relationship("Location", foreign_keys=[to_location_id], lazy=True)
    items = db.relationship("StockTransferItem", backref="transfer", lazy=True, cascade="all, delete-orphan")

    def insert(self):
        db.session.add(self)
        db.session.commit()
        return self


class StockTransferItem(PkModel):
    __tablename__ = "shopyo_ecommerce_stock_transfer_items"
    transfer_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_stock_transfers.id"), nullable=False)
    product_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_product.id"), nullable=False)
    quantity = db.Column(db.Integer, default=0)

    product = db.relationship("Product", lazy=True)

    def insert(self):
        db.session.add(self)
        db.session.commit()
        return self


class InventoryCount(PkModel):
    __tablename__ = "shopyo_ecommerce_inventory_counts"
    created_at = db.Column(db.DateTime, default=db.func.now())
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)
    notes = db.Column(db.Text)
    status = db.Column(db.String(20), default="draft")

    items = db.relationship("InventoryCountItem", backref="count", lazy=True, cascade="all, delete-orphan")

    def __repr__(self):
        return f"InventoryCount #{self.id} ({self.status})"

    def insert(self):
        db.session.add(self)
        db.session.commit()
        return self


class InventoryCountItem(PkModel):
    __tablename__ = "shopyo_ecommerce_inventory_count_items"
    count_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_inventory_counts.id"), nullable=False)
    product_id = db.Column(db.Integer, db.ForeignKey("shopyo_ecommerce_product.id"), nullable=False)
    expected_qty = db.Column(db.Integer, default=0)
    actual_qty = db.Column(db.Integer, default=0)

    product = db.relationship("Product", lazy=True)

    def insert(self):
        db.session.add(self)
        db.session.commit()
        return self

    @property
    def variance(self):
        return (self.actual_qty or 0) - (self.expected_qty or 0)
