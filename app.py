...
def create_app(config_name="development", instance_path=None):
    ...
    shopyo_ecommerce = ShopyoEcommerce(app)
...
def setup_flask_admin(app):
    import flask_admin
    flask_admin_version = Version(flask_admin.__version__)
    admin_kwargs = dict(app=app, name="My App", index_view=MyAdminIndexView())
    if flask_admin_version.major < 2:
